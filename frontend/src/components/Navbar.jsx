    ...
    <>
      <nav className="glass-navbar" style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
          ...
          {/* Wishlist — hidden on mobile (in hamburger drawer) */}
          <div style={{ position: 'relative' }} className="nav-wishlist-desktop">
          ...
          {/* Cart Icon — hidden on mobile (in bottom nav) */}
          <button onClick={() => navigate('/cart')} className="nav-action-btn nav-cart-desktop" style={{ position: 'relative' }} aria-label="Shopping Cart">
            <svg width="22" height="22" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
      ...
      <nav className="mobile-bottom-nav" aria-label="Mobile navigation">
        <button
          ...
          aria-label="Home"
        >
          <svg width="24" height="24" fill={currentPath === '/' ? 'var(--primary)' : 'none'} stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
        ...
        >
          {totalItems > 0 && (
            <span className="mobile-nav-badge">{totalItems > 9 ? '9+' : totalItems}</span>
          )}
          <svg width="24" height="24" fill="none" stroke={currentPath === '/cart' ? 'var(--primary)' : 'currentColor'} strokeWidth="2" viewBox="0 0 24 24">
            <circle cx="9" cy="21" r="1"></circle>
            <circle cx="20" cy="21" r="1"></circle>
            <path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6"></path>
          </svg>
          <span style={{ color: currentPath === '/cart' ? 'var(--primary)' : undefined }}>Cart</span>
        </button>
